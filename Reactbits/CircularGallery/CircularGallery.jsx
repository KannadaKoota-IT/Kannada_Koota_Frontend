...
class Media {
  ...
  update(scroll, direction) {
    ...
    if (this.program.uniforms.tMap.value.image?.tagName === "VIDEO") {
...
class App {
  ...
  createMedias({ items = [], bend, textColor, borderRadius, font }) {
    ...
    this.medias = mediaList.concat(mediaList).map((data, i) =>
      new Media({
        geometry: this.planeGeometry,
        gl: this.gl,
        image: data.image,
        index: i,
        length: mediaList.length * 2,
        renderer: this.renderer,
        scene: this.scene,
        screen: this.screen,
        text: data.text,
