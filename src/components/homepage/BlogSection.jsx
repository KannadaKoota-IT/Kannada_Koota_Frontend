  ...
  <div className="absolute inset-0 pointer-events-none opacity-40 z-0 mix-blend-screen">
    <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      <filter id="noiseFilter">
        ...
        <feColorMatrix type="matrix" values="1 0 0 0 0 0 0.8 0 0 0 0 0 0.2 0 0 0 0 0 1 0" />
      </filter>
...
const CornerMandala = ({ position }) => (
  <div className={`absolute w-[400px] h-[400px] md:w-[600px] md:h-[600px] pointer-events-none z-0 opacity-[0.08] ${position}`}>
    <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" className="w-full h-full text-yellow-600 fill-current animate-spin-slow">
      ...
      <circle cx="100" cy="100" r="45" stroke="currentColor" strokeWidth="1" fill="none" />
      <circle cx="100" cy="100" r="65" stroke="currentColor" strokeWidth="0.5" strokeDasharray="4 2" fill="none" />
      <path d="M100 10 Q140 10 140 50 T100 90 T60 50 T100 10" fill="none" stroke="currentColor" />
      <path d="M100 190 Q60 190 60 150 T100 110 T140 150 T100 190" fill="none" stroke="currentColor" />
      <path d="M10 100 Q10 140 50 140 T90 100 T50 60 T10 100" fill="none" stroke="currentColor" />
      <path d="M190 100 Q190 60 150 60 T110 100 T150 140 T190 100" fill="none" stroke="currentColor" />
    </svg>
  </div>
);
...
const BlogSection = () => {
  ...
  useEffect(() => {
    if (blogs.length > 0) {
      const sortedByDate = [...blogs].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
      const newestBlog = sortedByDate[0];
      const otherBlogs = sortedByDate.slice(1);
      const sortedByRating = otherBlogs.sort((a, b) => getAverageRating(b) - getAverageRating(a));
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.from(".blog-card-wrapper", {
        x: 100, opacity: 0, duration: 0.8, stagger: 0.15, ease: "power2.out",
  ...
  return (
    ...
    >
      {/* CSS Overrides */}
      <style jsx>{`
        .blog-card-wrapper h3, .blog-card-wrapper h4, .blog-card-wrapper .card-title {
      ...
      <div className="relative w-full max-w-[1400px] z-10 px-4 md:px-8">
        ...
        {!loading && processedBlogs.length > 0 && (
          <div className="relative group/carousel">
            ...
            <div
              ...
              className="blog-scroller no-scrollbar flex gap-6 overflow-x-auto snap-x snap-mandatory scroll-smooth py-10 px-2"
            >
              {processedBlogs.map((blog) => (
                // --- CARD WRAPPER WITH PRECISE WIDTHS ---
                // Mobile: 100% width
                // Tablet (md): 50% width minus half gap (to show 2)
                // Desktop (lg): 33.33% width minus gap ratio (to show exactly 3)
                <div
                  key={blog._id}
                  className="
                    blog-card-wrapper 
                ...
                >
                  ...
                  <div
                    className="
                      relative h-full flex flex-col min-h-[420px]
                      bg-black/80 backdrop-blur-sm 
                      rounded-2xl border border-[#fbbf24]/40 
                      overflow-hidden 
                      transition-transform duration-300 ease-out 
                      hover:scale-[1.02] hover:border-[#fbbf24]
        ...
        {!loading && processedBlogs.length === 0 && (
          <div className="text-center bg-black/40 p-10 rounded-2xl border border-[#fbbf24]/20 backdrop-blur-md max-w-lg mx-auto">
            <p className="text-2xl text-[#fbbf24] font-serif mb-2">No Announcements</p>
